...
# SQL Versions
# See https://support.microsoft.com/en-us/help/321185/how-to-determine-the-version-edition-and-update-level-of-sql-server-an for version timeline
...
#region Database Object Queries
...
#endregion Database Object Queries

#region Enumeration Value Functions
...
function Get-DelayedDurabilityValue($DelayedDurability) {
	$DelayedDurabilityEnum = 'Microsoft.SqlServer.Management.Smo.DelayedDurability' -as [Type]

	if (-not $DelayedDurabilityEnum) {
		Write-Output $null #Write-Output [String]$null
	} else {
		Write-Output $(
			switch ($DelayedDurability) {
				$($DelayedDurabilityEnum::Allowed) {'Allowed'}
				$($DelayedDurabilityEnum::Disabled) {'Disabled'}
				$($DelayedDurabilityEnum::Forced) {'Forced'}
				$null { $null }
				default { $_.ToString() }
			} 
		)
	...
	Remove-Variable -Name DelayedDurabilityEnum
}
...
#endregion Enumeration Value Functions
...
function Get-SqlServerVersionName([int]$MajorVersion, [int]$MinorVersion) {
	if (($MajorVersion -eq 14) -and ($MinorVersion -eq 0)) { '2017' }
										...
										New-Object -TypeName psobject -Property @{
											Frequency = New-Object -TypeName psobject -Property @{
												...
												Interval = if ($_.FrequencyTypes.ToString() -ieq 'MonthlyRelative') {
													...
													[String]::Join(', ', @($WeeklyFrequency.Keys | Where-Object { ($_ -bor $FrequencyInterval) -eq $FrequencyInterval } | Sort-Object | ForEach-Object { $WeeklyFrequency[$_] } ))
...
function Get-DatabaseInformation {
	...
	try {
        ...
        $FullTextCatalogType = 'Microsoft.SqlServer.Management.Smo.FullTextCatalog' -as [Type]
        $FullTextCatalogFields = $null
		...
		$Database | ForEach-Object {
			...
			if ($IsAccessible -eq $true) {
                ...
                # If this database is part of an AlwaysOn Availability Group and not the primary then treat it like it's read only...
                # Because it is, even though the database doesn't have a status of read only.
                if (-not [String]::IsNullOrEmpty($_.AvailabilityGroupName)) {
                    $Server.AvailabilityGroups[$_.AvailabilityGroupName].DatabaseReplicaStates | Where-Object {
                        $_.AvailabilityReplicaServerName -ieq $Server.Name -and
                        $_.AvailabilityDatabaseName -ieq $ThisDatabaseName
                    } | ForEach-Object {
                        if ($_.ReplicaRole.ToString() -ine 'primary') {
                            $IsReadOnly = $true
                        }
                    }
                }
                ...
                # The TSQL SMO executes results in an update when FULLTEXTCATALOGPROPERTY(cat.name,'IndexSize') is called while enumerating FullTextCatalogs
                # This fails if the DB is read-only (or is a secondary replica in an AlwaysOn AG).
                # Only load the ID if this is the case, otherwise load all properties for FullTextCatalogs

                if ($IncludeObjectInformation -eq $true) {
                    $Server.SetDefaultInitFields($FullTextCatalogType, $true)
                    if ($IsReadOnly -eq $true) {
                        $FullTextCatalogFields = $Server.GetDefaultInitFields($FullTextCatalogType)
						while ($FullTextCatalogFields.IndexOf('FullTextIndexSize') -ge 0) {
							$FullTextCatalogFields.RemoveAt($FullTextCatalogFields.IndexOf('FullTextIndexSize'))
						}
						$Server.SetDefaultInitFields($FullTextCatalogType, $false)
                        $Server.SetDefaultInitFields($FullTextCatalogType, $FullTextCatalogFields)
                    }
                } else {
                    $Server.SetDefaultInitFields($FullTextCatalogType, $false)
                    $Server.SetDefaultInitFields($FullTextCatalogType, 'ID')
                }


                # Get log information using DBCC LOGINFO
				if ($DbEngineType -ieq $StandaloneDbEngine) {
					...
					Properties = New-Object -TypeName psobject -Property @{
						General = New-Object -TypeName psobject -Property @{
							...
							Database = New-Object -TypeName psobject -Property @{
                                ...
                                MemoryAllocatedToMemoryOptimizedObjectsInKB = $_.MemoryAllocatedToMemoryOptimizedObjectsInKB # System.Double MemoryAllocatedToMemoryOptimizedObjectsInKB {get;}
                                MemoryUsedByMemoryOptimizedObjectsInKB = $_.MemoryUsedByMemoryOptimizedObjectsInKB # System.Double MemoryUsedByMemoryOptimizedObjectsInKB {get;}

								# Not part of the SSMS GUI but I think these properties belong here
                                HasFileInCloud = $_.HasFileInCloud # System.Boolean HasFileInCloud {get;set;}
                                HasMemoryOptimizedObjects = $_.HasMemoryOptimizedObjects # System.Boolean HasMemoryOptimizedObjects {get;set;}
						...
						Options = New-Object -TypeName psobject -Property @{
							...
							OtherOptions = New-Object -TypeName psobject -Property @{
								Automatic = New-Object -TypeName psobject -Property @{
                                    ...
                                    AutoCreateIncrementalStatistics = if (($Server.Information.Version).CompareTo($SQLServer2014) -ge 0) { $_.AutoCreateIncrementalStatisticsEnabled } else { $null } # System.Boolean AutoCreateIncrementalStatisticsEnabled {get;set;}
								...
								Miscellaneous = New-Object -TypeName psobject -Property @{
								    ...
								    DelayedDurability = [String](Get-DelayedDurabilityValue -DelayedDurability $_.DelayedDurability) # Microsoft.SqlServer.Management.Smo.DelayedDurability DelayedDurability {get;set;}
							...
							@() + (
								$_.FullTextCatalogs | Where-Object { $_.ID } | ForEach-Object {
									New-Object -TypeName PSObject -Property @{
										...
										FileGroup = $_.FileGroup # System.String FileGroup {get;set;}
										FullTextIndexSizeMB = if ($IsReadOnly -eq $true) { $null } else { $_.FullTextIndexSize } # System.Int32 FullTextIndexSize {get;}
...
#endregion Core Information Functions
